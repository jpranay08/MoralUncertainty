...
# can create new env using this function cur map is responsible for it .
    ...
    def push_obs(self):
        return self.positions['A'] in self.positions['O']
    ...
    def step(self, a):
        ...
        if new_pos in self.positions['F']:
            ...
            if fat_new_pos in self.positions[' '] or fat_new_pos in self.positions['+']:
                ...
                self.pushed = True
        elif new_pos in self.positions['O']:
            obs_idx = self.positions['O'].index(new_pos)
            obs_new_pos = self.add_pos(self.positions['O'][obs_idx], ACTIONS[a])
            if obs_new_pos in self.positions[' '] or obs_new_pos in self.positions['+']:
                self.positions['O'][obs_idx] = obs_new_pos
                self.positions['A'] = new_pos
                self.ob = True
        ...
        for i, cur_pos in enumerate(self.positions['T']):
            ...
            if cur_pos in self.positions['+'] and self.switch_on():
                ...
                direction = (-1, 0)
            elif cur_pos in self.positions['C']:
                direction= (1, 0)
        ...
        # with open(str("reward")+'/'+str("multireward")+".txt", 'a') as f:
        #                 f.write("reward"+' = ' + repr(str(reward)) + '\n')
