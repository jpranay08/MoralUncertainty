...
from dempster import getDempsterCred
...
class NashEnv:
    ...
    def step(self, orig_actions, verbose=False):
        ...
        weighted_actions = actions ** self.credences[:, None]
        votes = np.sum(weighted_actions * self.credences[:, None], axis=0)
        if verbose:
            print(votes)
...
class VarianceModel:
    def __init__(self, theories, get_credences, env, get_epsilon, model_type, credence_round,
        ...
        if model_type == 'tabular':
            ...
            print("----------------------------------------------")
            print("input theories", len(theories), theories )
            print("input obs space", env.observation_space.shape[0] )
    ...
    def predict(self, obs, add=False, deterministic=False, verbose=False):
        ...
        weighted_actions = (normalized_scores+100) ** self.credences[:, None]
        votes = np.sum(weighted_actions * self.credences[:, None], axis=0)
        if self.stochastic:
...
class FreeformVoter:
    ...
    def _get_trolley_model(self, is_testing):
        if self.env_args['credences'] is not None:
            ...
            def _get_cred():
                ...
                # probs = arr / arr.sum()
                a = round( np.random.rand(),2)
                b=round(random.uniform(0,1-a),2)
                c=round(1-a-b,2)

                probs = np.array(getDempsterCred([a, b,c]))
                #probs = np.array([a, b,c])
    ...
    def train_trolley(self, level='classic', on_track=10, on_track_dist='oneto', voting='nash',
                      theories=({'pushed_harms':-4,'collateral_harms':-1, 'lies': -0.5, 'doomsday': -10,'self_harms':-1},{'self_harms' : -0.1, 'doomsday' : -300,  'harms':-5},{'self_harms' : -1, 'doomsday' : -300,  'harms':-1}),
    ...
    def train_uniform(self, n_agents=3, n_actions=4, episode_steps=20, know_other_preferences=False, stochastic_voting=False,
              seed=-1, num_timesteps=10000, cost_exponent=2, mean_of_std=0.3, std_of_mean=0.1, nenvs=96,
